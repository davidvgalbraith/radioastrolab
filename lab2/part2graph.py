...
def main(argv):
   ...
   d["0"] = "high"
   d["1"] = "low"
   #data = np.fromfile("roach/twelve/mix_bram_" + d[argv[0]] + "_sig_5pct_12dbm", dtype='>i4')
   #data = np.fromfile("roach/mix_bram_" + d[argv[0]] + "_sig_5pct", dtype=">i4")
   data = np.fromfile("cosin/sin_bram", dtype=">i4")
   print len(data)
